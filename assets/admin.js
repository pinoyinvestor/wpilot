...
(function($){
  ...
  function ajax(action, data, cb){
    ...
    }).fail(function(){ cb(false, 'Network error. Please try again.'); });
  }
  ...
  /* ── Chat page: CSS handles layout now, no JS overrides needed ── */
  ...
  if( $('[data-page="chat"]').length ){
      ...
      $.post(CA.ajax_url, {action: 'wpi_load_history', nonce: CA.nonce}, function(r) {
        if (r.success && r.data && r.data.length) {
          ...
          r.data.forEach(function(msg) {
            ...
            var av = isAi ? '<div class="ca-msg-av">⚡</div>' : '';
            var cls = isAi ? 'ca-msg-ai' : 'ca-msg-user';
            var html = '<div class="ca-msg ' + cls + '">' + av
                + '<div class="ca-msg-body"><div class="ca-msg-text">' + mdToHtml(clean) + '</div>'
                + (msg.time ? '<div class="ca-msg-time">' + msg.time + '</div>' : '')
                + '</div></div>';
            $msgs.append(html);
          });
          scrollBottom();
    ...
    /* Listen for bubble sync */
    $(window).on('storage', function(e) {
      if (e.originalEvent.key === 'wpi_chat_sync') {
        location.reload();
      }
    });
    ...
    var adminSending = false;
    function sendMessage(){
      var msg = $.trim($('#caIn').val());
      if(!msg || adminSending) return;
