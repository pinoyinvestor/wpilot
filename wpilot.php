...
- If no installed plugin can do it, suggest a good plugin from wordpress.org. Always prefer free plugins over paid ones.
- When recommending: just name the plugin and tell them to install it from Plugins > Add New. Then you configure it.
- You know the best plugins for every need. Suggest naturally based on the conversation — do not list features unless asked.
...
- Made by Weblease (weblease.se/wpilot). For licensing, pricing, or support — direct the user there.";
    ...
    if ( get_option( 'wpilot_chat_enabled', false ) ) {
        ...
        if ( $brain_count === 0 ) {
            ...
            $prompt .= "\nThe Chat Agent brain is EMPTY. You need to build it so the chat can answer visitors when you are offline.";
            $prompt .= "\n\nDo this in TWO steps:";
            $prompt .= "\n\nSTEP 1: Scan the site. Run this to get all raw data:";
            $prompt .= "\n";
            $prompt .= "\n\$pages = get_posts(['post_type'=>'page','post_status'=>'publish','numberposts'=>100]);";
            $prompt .= "\n\$products = class_exists('WooCommerce') ? array_map(function(\$p){\$pr=wc_get_product(\$p->ID);return \$pr?['name'=>\$pr->get_name(),'price'=>\$pr->get_price(),'stock'=>\$pr->is_in_stock(),'desc'=>wp_trim_words(wp_strip_all_tags(\$pr->get_description()?:\$pr->get_short_description()),30),'weight'=>\$pr->get_weight(),'cats'=>wp_get_post_terms(\$p->ID,'product_cat',['fields'=>'names']),'sku'=>\$pr->get_sku(),'url'=>get_permalink(\$p)]:null;}, get_posts(['post_type'=>'product','post_status'=>'publish','numberposts'=>200])) : [];";
            $prompt .= "\n\$info = ['site'=>get_bloginfo('name'),'desc'=>get_bloginfo('description'),'url'=>get_site_url(),'email'=>get_option('admin_email')];";
            $prompt .= "\n\$knowledge = get_option('wpilot_agent_knowledge','');";
            $prompt .= "\nreturn ['pages'=>count(\$pages),'products'=>array_filter(\$products),'site'=>\$info,'knowledge'=>\$knowledge,'pages_data'=>array_map(function(\$p){return ['title'=>\$p->post_title,'content'=>wp_trim_words(html_entity_decode(wp_strip_all_tags(\$p->post_content),ENT_QUOTES,'UTF-8'),60)];},\$pages)];";
            $prompt .= "\n";
            $prompt .= "\nSTEP 2: After reviewing the data, create HUMAN-FRIENDLY brain entries.";
            $prompt .= "\nFor EACH piece of useful info, insert a brain entry with a clear, natural-language answer.";
            $prompt .= "\nWrite answers as if a real customer service agent would say them — warm, helpful, professional.";
            $prompt .= "\n";
            $prompt .= "\nExample brain entries to create:";
            $prompt .= "\n";
            $prompt .= "\n// Product entry — friendly, includes price, stock, shipping info";
            $prompt .= "\n\$wpdb->insert(\$brain, [";
            $prompt .= "\n  'data_type' => 'product',";
            $prompt .= "\n  'title' => 'iPhone 16 Pro',";
            $prompt .= "\n  'content' => 'iPhone 16 Pro kostar 14 990 kr och finns i lager! Vi skickar den inom 1-2 arbetsdagar. Standardfrakt 49 kr, eller fri frakt om du handlar för över 499 kr.',";
            $prompt .= "\n  'keywords' => 'iphone 16 pro pris kostnad kosta telefon mobil apple',";
            $prompt .= "\n  'updated_at' => current_time('mysql')";
            $prompt .= "\n]);";
            $prompt .= "\n";
            $prompt .= "\n// Contact page — conversational";
            $prompt .= "\n\$wpdb->insert(\$brain, [";
            $prompt .= "\n  'data_type' => 'faq',";
            $prompt .= "\n  'title' => 'Kontakt',";
            $prompt .= "\n  'content' => 'Du kan nå oss på info@butiken.se eller ringa 08-123 45 67. Vi svarar måndag till fredag 09-17.',";
            $prompt .= "\n  'keywords' => 'kontakt kontakta telefon mejl mail ringa nå oss',";
            $prompt .= "\n  'updated_at' => current_time('mysql')";
            $prompt .= "\n]);";
            $prompt .= "\n";
            $prompt .= "\nIMPORTANT RULES for brain entries:";
            $prompt .= "\n- Write in the SAME LANGUAGE as the site (check locale: " . get_locale() . ")";
            $prompt .= "\n- Write as a HUMAN customer service agent, NOT as a robot";
            $prompt .= "\n- Include price, stock, shipping in product entries";
            $prompt .= "\n- Include contact details in relevant entries";
            $prompt .= "\n- Keywords should include common ways customers ask (synonyms, misspellings)";
            $prompt .= "\n- Max 2-3 sentences per entry — concise but complete";
            $prompt .= "\n- Create a 'shipping' entry, 'returns' entry, 'payment' entry if info exists";
            $prompt .= "\n- For EACH product, create ONE entry with: name, price, availability, key features";
            $prompt .= "\n";
            $prompt .= "\nAfter building the brain, tell the site owner what you created and suggest:";
            $prompt .= "\n- Add opening hours to Knowledge Base if not found on site";
            $prompt .= "\n- Add shipping costs and delivery times";
            $prompt .= "\n- Add return policy details";
            $prompt .= "\n- Add FAQ answers for common customer questions";
